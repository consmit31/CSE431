...
for i in range(idx):
    ...
    if opr == "a":
        if median == None and gt.size == 0 and lt.size == 0:
           median = num
        elif gt.size == 0 and lt.size == 0:
            if num > median:
                gt.insert(num)
                lt.insert(median)
            else:
            ...
            if gt.size == lt.size:
                    median = (gt.maximum().get_key() + lt.minimum().get_key()) / 2
            elif lt.size > gt.size:
                median = lt.maximum().get_key()
            else:
                median = gt.minimum().get_key()

        print("{:f}".format(median).rstrip('0').rstrip('.'))

    elif opr == "r":
        if median == None and gt.size == 0 and lt.size == 0:
            print("Wrong!")
        else:
            if lt.search(num) == False or gt.search(num) == False:
                print("Wrong!")
            elif num > median:
                gt.delete(num)
            else:
            ...
            if gt.size == 0 and lt.size == 0:
                print("Wrong!")
            else:
                if gt.size == lt.size:
                    median = (gt.minimum().get_key() + lt.maximum().get_key()) / 2
                elif lt.size > gt.size:
                    median = lt.maximum().get_key()
                else:
                    median = gt.minimum().get_key()
